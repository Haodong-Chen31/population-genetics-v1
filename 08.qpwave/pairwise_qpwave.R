...
setwd('D:/R/Rworking/popgenetics/ganqing/sample_homo5/')
...
#annotation_row <- data.frame(goup=poplist$V2)
#rownames(annotation_row) <- poplist$V1
#annotation_col <- annotation_row
#画图
ht <- pheatmap(qpwave_matrix, display_numbers = annotation, main = 'pairwise qpWave rank=0', cutree_rows=5, cutree_cols = 5,
               fontsize_number = 20,cellwidth = 40,cellheight = 40,fontsize=20,clustering_method = "ward.D2",
...
pdf("pairwise_qpwave_studiedpop3.pdf",width =16,height =16)
print(ht)
dev.off()
...
#                    fontsize_number = 14,cellwidth = 80,cellheight = 80,
#                    display_numbers = annotation_extract, main = paste0(pop,'-pairwise qpWave rank=0'),
#                    gaps_row = 1, gaps_col = 1,breaks = breaks,
#                    cutree_rows = 2, cutree_cols = 2
#                    #annotation_row = annotation_row, annotation_col = annotation_col
#                    )
#   } else {
#     ht <- pheatmap(qpwave_matrix_extract, display_numbers = annotation_extract, main = paste0(pop,'-pairwise qpWave rank=0'),
#                    fontsize = 30,cellwidth = 40,cellheight = 40,fontsize_number = 26,
#                    cutree_rows = 2, cutree_cols = 2
#                    )
#   }
#   pdf(paste0(pop,"_pairwise_qpwave.pdf"), width = 26, height = 26)
#   print(ht)
#   dev.off()
# }