...
folder <- file.path(path=paste0(workdir,"/test_migrations"))                     #path to files of TreeMix replicates with different migration edges (m) to test
...
setwd(paste0(workdir,"/final_runs"))                                             #folder with all TreeMix outputs from the final runs
#要改的
maxLL(paste0("treemix_",mig,"m_finalrun_"), nt=runs)                                         #first argument is stem of TreeMix output files, nt = number of runs
...
#要改的
cfTrees(paste0("treemix_",mig,"m_finalrun_"), nt=runs, p=1, m='PH85')                        #m is the method to calculate pairwise distances between unique trees (default is Robinson-Foulds distance)
...
#改 画最高likelihood的（TreeLLs.txt），改下面的文件名

TreeLLs <- read.table("TreeLLs.txt",header=T)
treemix.bootstrap(paste0("treemix_",mig,"m_finalrun_",TreeLLs[1,1]), out.file = "tmp",                 #stem of TreeMix files (as above) + number of run with highest likelihood and unique topology
...
treemix.drift(paste0("treemix_",mig,"m_finalrun_",TreeLLs[1,1]),                             #pairwise matrix for drift estimates with specified order of pops 
              pop.order.color.file = "poporder.txt") + 
              title("Drift")     

plot_resid(paste0("treemix_",mig,"m_finalrun_",TreeLLs[1,1]),                                          #pairwise matrix for residuals
...
#改
GetMigrStats(input_stem=paste0("treemix_",mig,"m_finalrun_"), nt=runs)                       #arguments as above, writes file "MS_and_stats.txt" into current directory
...
setwd(paste0(workdir,"/final_runs/bootstrap"))

#Copy PairsOfSets.txt into directory
file.copy(
  list.files(dirname(getwd()), pattern="PairsOfSets.txt", full.names=TRUE),
  getwd(),
  overwrite=TRUE
)
